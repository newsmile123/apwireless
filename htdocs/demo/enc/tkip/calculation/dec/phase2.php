<?php
session_start();
if (isset($_POST['ph2_ttak']) && isset($_POST['ph2_tk']) && isset($_POST['ph2_tsc16'])){
	$ttak = str_replace(" ", "", $_POST['ph2_ttak']);
	$tk = str_replace(" ", "", $_POST['ph2_tk']);
	$tsc16 = str_replace(" ", "", $_POST['ph2_tsc16']);
}
// Шаг 1
for ($i = 0; $i < 5; $i++){
	$ppk[$i] = $ttak[$i];
...
print_r($wepSeed);
// Конец шага 3
$_SESSION['demo']['2']['tkip']['var'] = "1";
$_SESSION["demo"]['2']["tkip"]["seed"] = $wepSeed;
$_SESSION['demo']['2']['tkip']['wep']["key"] = substr(implode(" ", $_SESSION["demo"]["tkip"]["seed"]), 6, 32);
$_SESSION['demo']['2']['tkip']['wep']["iv"] = substr(implode(" ", $_SESSION["demo"]["tkip"]["seed"]), 0, 6);
