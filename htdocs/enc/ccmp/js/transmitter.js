...
{
var myWindow;
myWindow = window.open("construct_aad.php","", "width=1000,height=400");
}
function open_nonce()
{
var myWindow;
myWindow = window.open("construct_nonce.php","", "width=930,height=450");
}
...
function aad()
		{
			...
			if (fc == 'Frame Control' && a1 == 'Address 1' && a2 == 'Address 2' && a3 == 'Address 3' && a4 == 'Address 4' && sc == 'Sequence Control' && qos == 'QoS Control'){
				...
				var posting = $.post("aad.php", { fc: fc, a1: a1, a2: a2, a3: a3, a4: a4, sc: sc, qos: qos })
			...
			if (cipherkey == ''){
				alert("Введите ключ шифрования");
					...
					if (regex.test(cipherkey) && cipherkey.length == 32){
						$.post("ctr.php", { cipherkey: cipherkey },
							function(result){
									$("#ciphertext").empty();
...
function cbc()
		{
			...
			if (cipherkey == ''){
				alert("Введите ключ шифрования");
					...
					if (regex.test(cipherkey) && cipherkey.length == 32){
			$.post("cbcmac.php", { cipherkey: cipherkey },
				function(result){
					$("#code").empty();
					$('#code').html(result);
			});
